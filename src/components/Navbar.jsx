            ...
            <div className='flex justify-between items-center'>
                ...
                <div className='hidden md:flex space-x-8 border border-gray-200 rounded-full px-10 py-4'>
                    {navMenu.map((item, index) => (
                        <a key={index} href={`#${item}`} onClick={e => {e.preventDefault(); document.getElementById(item)?.scrollIntoView({behavior: 'smooth'});}}>{item}</a>
                    ))}
                </div>
                {/* Buttons */}
                <div>
                    ...
                    >
                        Resume
                        <FaArrowRight className='text-gray-500 tex-sm' />
                    </a>
                </div>
            </div>
        </div>
